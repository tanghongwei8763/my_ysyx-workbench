...
VReg::VReg(const char* _vcname__)
    : VReg(Verilated::threadContextp(), _vcname__)
{
}
...
#endif  // VL_DEBUG
void VReg___024root___eval_static(VReg___024root* vlSelf);
...
void VReg::eval_step() {
    ...
    if (VL_UNLIKELY(!vlSymsp->__Vm_didInit)) {
        ...
        VReg___024root___eval_static(&(vlSymsp->TOP));
...
//============================================================
// Events and timing
bool VReg::eventsPending() { return false; }

uint64_t VReg::nextTimeSlot() {
...
void VReg___024root___eval_final(VReg___024root* vlSelf);

VL_ATTR_COLD void VReg::final() {
    VReg___024root___eval_final(&(vlSymsp->TOP));
}

//============================================================
// Implementations of abstract methods from VerilatedModel

const char* VReg::hierName() const { return vlSymsp->name(); }
const char* VReg::modelName() const { return "VReg"; }
unsigned VReg::threads() const { return 1; }
std::unique_ptr<VerilatedTraceConfig> VReg::traceConfig() const {
...
void VReg___024root__trace_init_top(VReg___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    VReg___024root__trace_init_top(vlSelf, tracep);
...
VL_ATTR_COLD void VReg::trace(VerilatedVcdC* tfp, int levels, int options) {
    if (tfp->isOpen()) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'VReg::trace()' shall not be called after 'VerilatedVcdC::open()'.");
    }
    if (false && levels && options) {}  // Prevent unused
    tfp->spTrace()->addModel(this);
    tfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    VReg___024root__trace_register(&(vlSymsp->TOP), tfp->spTrace());
}
