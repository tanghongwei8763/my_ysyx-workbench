...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = (unsigned int)src1 / (unsigned int)src2);
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = src1 * src2);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (((int64_t)(int)src1 * (int64_t)(int)src2) >> 32));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = (((int64_t)(int)src1 * (uint64_t)(int)src2) >> 32));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (((uint64_t)src1 * (uint64_t)src2) >> 32););
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(rd) = src1 | src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = (int)src1 % (int)src2);
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(rd) = (unsigned int)src1 % (unsigned int)src2);
  ...
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = ((unsigned int)src1 < (unsigned int)src2));
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = (int)src1 >> (src2 & 0x1f));
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(rd) = (unsigned int)src1 >> (src2 & 0x1f));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4;s->dnpc = src1 + imm;
                                                                ...
                                                                ret_func(s->pc);));
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = (int)((char)(Mr(src1 + imm, 1))));
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = ((unsigned int)src1 < (unsigned int)imm));//seqz
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (int)src1 >> imm);//有符号数
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = (unsigned int)src1 >> imm);//无符号数
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc = ((int)src1 == (int)src2) ? (s->pc + imm) : (s->snpc));//beqz
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = ((int)src1 >= (int)src2) ? (s->pc + imm) : (s->snpc));
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, s->dnpc = ((unsigned int)src1 >= (unsigned int)src2) ? (s->pc + imm) : (s->snpc));
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, s->dnpc = ((unsigned int)src1 < (unsigned int)src2) ? (s->pc + imm) : (s->snpc));
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc = ((int)src1 < (int)src2) ? (s->pc + imm) : (s->snpc));
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, s->dnpc = ((int)src1 != (int)src2) ? (s->pc + imm) : (s->snpc));//bnez
